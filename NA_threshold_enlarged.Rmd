...
holdout = holdout %>% 
  # mutate(metro = ifelse(grepl("Metro", rucc_category) , 1, 0),
  #        metro = as.factor(metro) ) %>% 
  select_at( setdiff(all_feature, new_feature) ) %>%
  mutate_at(setdiff(cat_idx2, new_feature), as.factor) %>% 
...
## imputation
# train_holdout = rbind(cbind(all_selected[,colnames(holdout)],dataset = "train"),
...
```{r}
## feature engineering
holdout_imp_final = holdout_imp %>% 
  mutate(metro = ifelse(grepl("Metro", rucc_category) , 1, 0),
         metro = as.factor(metro) )
save(holdout_imp_final, file = "holdout_imputation.Rdata")
```
